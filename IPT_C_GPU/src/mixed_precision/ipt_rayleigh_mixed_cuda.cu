...
#include "ipt_mixed_precision_cuda.cu"
...
static int rayleigh_orthogonalize_qr(double *d_q, int n, int k)
{
    ...
    solver_status = cusolverDnDorgqr_bufferSize(solver, n, k, k, d_q, n, d_tau,
                                                &lwork_orgqr);
    if (solver_status != CUSOLVER_STATUS_SUCCESS) {
        status = IPT_CUDA_CUBLAS_ERROR;
        goto cleanup;
    ...
    solver_status =
        cusolverDnDorgqr(solver, n, k, k, d_q, n, d_tau, d_work, lwork, d_info);
    if (solver_status != CUSOLVER_STATUS_SUCCESS) {
        status = IPT_CUDA_CUBLAS_ERROR;
        goto cleanup;
    }
    ...
    cublasHandle_t handle, IPTRayleighMixedCudaDeviceResult *result)
{
    ...
    stage_start = ipt_mixed_now_seconds();
    /* TF32 IPT columns are already close to normalized; QR handles scaling. */
    status = rayleigh_orthogonalize_qr(d_q, n, k);
    if (status != IPT_CUDA_SUCCESS) {
        goto cleanup;
    }
    RAYLEIGH_CUDA_CHECK(cudaDeviceSynchronize());
