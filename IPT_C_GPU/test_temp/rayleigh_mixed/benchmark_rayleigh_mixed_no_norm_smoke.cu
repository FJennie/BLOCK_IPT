...
#define DEFAULT_NS "4096,8192,16384,26000"
...
static void load_config(Config *cfg)
{
    ...
    if (summary != NULL && summary[0] != '\0') {
        ...
        snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                 "%s/rayleigh_mixed_no_norm_smoke_summary.txt",
                 cfg->results_dir);
    }
}
                            ...
                            size_t size)
{
    ...
    snprintf(path, size, "%s/rayleigh_mixed_no_norm_smoke_epsilon_%s.csv",
                           ...
                           const char *gpu_name)
{
    ...
    fprintf(fp,
            "%d,%.12g,rayleigh_mixed_no_norm,%.17g,%.17g,%d,%d,%d,"
...
int main(void)
{
    ...
    snprintf(summary_line, sizeof(summary_line),
             "started_at=%s method=rayleigh_mixed_no_norm repeats=%d tf32_tol=%.17g "
             "fp64_tol=%.17g gpu=%s",
