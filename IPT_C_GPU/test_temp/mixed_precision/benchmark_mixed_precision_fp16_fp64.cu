...

    {
        ...
        if (result_csv != NULL && result_csv[0] != '\0') {
            ...
            snprintf(cfg->result_csv, sizeof(cfg->result_csv),
                     "%s/mixed_precision_fp16_fp64_results.csv",
                     cfg->results_dir);
        }
        ...
        if (summary_txt != NULL && summary_txt[0] != '\0') {
            ...
            snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                     "%s/mixed_precision_fp16_fp64_summary.txt",
                     cfg->results_dir);
        }
    }
}
                             ...
                             int compute_residuals)
{
    ...
    status = ipt_mixed_precision_cuda_device_tol(
        d_matrix, cfg->n, cfg->n, cfg->fp16_tol, cfg->fp16_maxiter,
...
static void write_header(const Config *cfg)
{
    ...
    fprintf(fp,
            "N,epsilon,fp16_tol,fp64_tol,fp16_maxiter,fp64_maxiter,"
            "repeats,time_fp16_initial_sec,time_fp64_refine_sec,"
            "time_total_sec,final_residual,fp16_fixed_point_residual,"
                       ...
                       const char *status, const char *error)
{
    ...
    int fp16_converged = isfinite(result->fp16_fixed_point_residual) &&
                         result->fp16_fixed_point_residual <= cfg->fp16_tol;
    ...
    fprintf(fp,
            ...
            cfg->n, epsilon, cfg->fp16_tol, cfg->fp64_tol,
            cfg->fp16_maxiter, cfg->fp64_maxiter, cfg->repeats,
            result->fp16_time_sec, result->fp64_time_sec,
            result->total_time_sec, result->residual,
            result->fp16_fixed_point_residual,
            result->fp64_fixed_point_residual, result->fp16_iterations,
            result->fp64_iterations, bool_text(fp16_converged),
...
static void write_summary_header(const Config *cfg, const char *gpu_name)
{
    ...
    fprintf(fp, "IPT_C mixed precision benchmark: FP16 initial + FP64 refine\n");
...
int main(void)
{
    ...
    for (int idx = 0; idx < cfg.eps_count; ++idx) {
        ...
        printf("Mixed average: fp16 = %.8g s, fp64 = %.8g s, "
               "total = %.8g s, residual = %.8g, fp16_fp = %.8g, "
               "fp64_fp = %.8g, fp16_iters = %d, fp64_iters = %d, "
               "status = %s\n",
               mixed.fp16_time_sec, mixed.fp64_time_sec,
    ...
    row_done:
        ...
        snprintf(summary_line, sizeof(summary_line),
                 ...
                 "time_fp16=%.17g time_fp64=%.17g time_total=%.17g "
                 "fp16_iterations=%d fp64_iterations=%d fp16_fp=%.17g "
                 "fp64_fp=%.17g final_residual=%.17g repeats=%d "
                 "per_epsilon_warmup=true",
                 cfg.n, epsilon, time_buf, status, mixed.fp16_time_sec,
                 mixed.fp64_time_sec, mixed.total_time_sec,
                 mixed.fp16_iterations, mixed.fp64_iterations,
                 mixed.fp16_fixed_point_residual,
