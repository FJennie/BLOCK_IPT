...

    {
        ...
        if (summary != NULL && summary[0] != '\0') {
            ...
            snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                     "%s/fp16_cusolver_sweep_summary.txt", cfg->results_dir);
        }
    }
}
                            ...
                            size_t size)
{
    ...
    snprintf(path, size, "%s/fp16_cusolver_sweep_epsilon_%s.csv",
...
static void write_csv_header(const char *csv_path)
{
    ...
    fprintf(fp,
            "N,epsilon,fp16_tol,fp64_tol,fp16_maxiter,fp64_maxiter,repeats,"
            "time_cusolver_syevd_sec,residual_cusolver_syevd,"
            "time_fp16_initial_sec,time_fp64_refine_sec,time_mixed_total_sec,"
            "residual_mixed,fp16_fixed_point_residual,"
            "fp64_fixed_point_residual,fp16_iterations,fp64_iterations,"
            "fp16_converged,fp64_converged,gpu_name,status,error\n");
    fclose(fp);
}

static void append_csv_row(const char *csv_path, const Config *cfg, int n,
                           double epsilon, const SolverResult *syevd,
                           const MixedResult *mixed, const char *gpu_name,
                           const char *status, const char *error)
{
    ...
    int fp16_converged = isfinite(mixed->fp16_fixed_point_residual) &&
                         mixed->fp16_fixed_point_residual <= cfg->fp16_tol;
    ...
    fprintf(fp,
            "%d,%.12g,%.17g,%.17g,%d,%d,%d,%.17g,%.17g,%.17g,%.17g,"
            "%.17g,%.17g,%.17g,%.17g,%d,%d,%s,%s,%s,%s,%s\n",
            n, epsilon, cfg->fp16_tol, cfg->fp64_tol, cfg->fp16_maxiter,
            cfg->fp64_maxiter, cfg->repeats, syevd->time_sec,
            syevd->residual, mixed->fp16_time_sec, mixed->fp64_time_sec,
            mixed->total_time_sec, mixed->residual,
            mixed->fp16_fixed_point_residual,
            mixed->fp64_fixed_point_residual, mixed->fp16_iterations,
            mixed->fp64_iterations, bool_text(fp16_converged),
...
static void write_summary_header(const Config *cfg, const char *gpu_name)
{
    ...
    fprintf(fp, "IPT_C mixed precision sweep: FP16 initial + FP64 refine vs "
                "cuSolver Dsyevd\n");
...
int main(void)
{
    ...
    printf("===== IPT_C mixed precision FP16 sweep vs cuSolver =====\n");
    printf("ROOT = %s\n", cfg.root);
    printf("RESULT_DIR = %s\n", cfg.results_dir);
    ...
    for (int eps_idx = 0; eps_idx < cfg.eps_count; ++eps_idx) {
        ...
        for (int n_idx = 0; n_idx < cfg.n_count; ++n_idx) {
            ...
            printf("Averages: syevd = %.8g s, mixed_total = %.8g s, "
                   "mixed_residual = %.8g, fp16_fp = %.8g, fp64_fp = %.8g, "
                   "fp16_iters = %d, fp64_iters = %d, status = %s\n",
                   syevd.time_sec, mixed.total_time_sec, mixed.residual,
                   mixed.fp16_fixed_point_residual,
                   mixed.fp64_fixed_point_residual, mixed.fp16_iterations,
                   mixed.fp64_iterations, status);

        row_done:
            append_csv_row(csv_path, &cfg, n, epsilon, &syevd, &mixed,
                           gpu_name, status, error);
            wall_time_string(time_buf, sizeof(time_buf));
            snprintf(summary_line, sizeof(summary_line),
                     "epsilon=%.12g N=%d finished_at=%s status=%s "
                     "time_syevd=%.17g residual_syevd=%.17g "
                     "time_fp16=%.17g time_fp64=%.17g time_mixed=%.17g "
                     "residual_mixed=%.17g fp16_iterations=%d "
                     "fp64_iterations=%d fp16_fp=%.17g fp64_fp=%.17g "
                     "repeats=%d per_case_warmup=true csv=%s",
                     epsilon, n, time_buf, status, syevd.time_sec,
                     syevd.residual, mixed.fp16_time_sec,
                     mixed.fp64_time_sec, mixed.total_time_sec,
                     mixed.residual, mixed.fp16_iterations,
                     mixed.fp64_iterations,
                     mixed.fp16_fixed_point_residual,
                     mixed.fp64_fixed_point_residual, cfg.repeats, csv_path);
            append_summary(&cfg, summary_line);
            cudaFree(d_matrix);
            free(h_matrix);
            cudaDeviceSynchronize();
