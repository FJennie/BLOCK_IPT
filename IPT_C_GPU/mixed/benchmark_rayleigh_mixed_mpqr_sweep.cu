...
#include "ipt_rayleigh_mixed_mpqr_cuda.cu"

#define DEFAULT_ROOT "/fs1/home/nudt_liujie/ftt/IPT_C_GPU"
#define DEFAULT_NS "4096"
#define DEFAULT_EPS_VALUES "0.09,0.10,0.11,0.12"
...
static void load_config(Config *cfg)
{
    ...
    if (summary != NULL && summary[0] != '\0') {
        ...
        snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                 "%s/rayleigh_mixed_mpqr_sweep_summary.txt",
                 cfg->results_dir);
    }
}
                                              ...
                                              const Config *cfg)
{
    ...
    printf("Rayleigh mixed MPQR warmup for N=%d\n", n);
    {
                            ...
                            size_t size)
{
    ...
    snprintf(path, size, "%s/rayleigh_mixed_mpqr_sweep_epsilon_%s.csv",
                           ...
                           const char *gpu_name)
{
    ...
    fprintf(fp,
            "%d,%.12g,rayleigh_mixed_mpqr,%.17g,%.17g,%d,%d,%d,"
...
int main(void)
{
    ...
    snprintf(summary_line, sizeof(summary_line),
             "started_at=%s method=rayleigh_mixed_mpqr repeats=%d tf32_tol=%.17g "
             "fp64_tol=%.17g gpu=%s",
             time_buf, cfg.repeats, cfg.tf32_tol, cfg.fp64_tol, gpu_name);
    append_summary(&cfg, summary_line);

    printf("===== Rayleigh mixed QR sweep =====\n");
