...
static void load_config(Config *cfg)
{
    ...
    cfg->results_dir = env_or_default("ACX_MIXED_RAYLEIGH_RESULTS_DIR",
                                      "ACX_MIXED_RESULTS_DIR", NULL);
    cfg->logs_dir =
        env_or_default("ACX_MIXED_RAYLEIGH_LOG_DIR", "ACX_MIXED_LOG_DIR",
                       NULL);
                                   ...
                                   const Config *cfg)
{
    ...
    printf("ACX mixed Rayleigh TF32 warmup for N=%d\n", n);
    {
    ...
    for (int rep = 1; rep <= cfg->repeats; ++rep) {
        ...
        printf("  ACX mixed Rayleigh TF32 run %d/%d: total=%.8g s, "
               "tf32=%.8g s, orth=%.8g s, rayleigh=%.8g s, "
               "fp64=%.8g s, final=%.8g s, back=%.8g s, "
               ...
               one.orthogonalize_time_sec, one.rayleigh_transform_time_sec,
               one.fp64_time_sec, one.finalization_time_sec,
               one.backtransform_time_sec, one.tf32_fixed_point_residual,
                            ...
                            size_t size)
{
    ...
    snprintf(path, size,
             "%s/acx_mixed_rayleigh_tf32_cuda_sweep_epsilon_%s.csv",
...
static void write_csv_header(const char *csv_path)
{
    ...
    fprintf(fp,
            ...
            "time_backtransform_sec,time_total_sec,absolute_residual,"
            "fp64_max_residual,"
                           ...
                           const char *gpu_name)
{
    ...
    fprintf(fp,
            "%d,%.17g,acx_mixed_rayleigh_tf32_fp64,%.17g,%.17g,%d,%d,%d,"
            ...
            result->finalization_time_sec, result->backtransform_time_sec,
...
int main(void)
{
    ...
    for (int eps_idx = 0; eps_idx < cfg.eps_count; ++eps_idx) {
        ...
        row_done:
            ...
            wall_time_string(time_buf, sizeof(time_buf));
            snprintf(summary_line, sizeof(summary_line),
                     "epsilon=%.12g N=%d method=acx_mixed_rayleigh_tf32_fp64 "
                     "finished_at=%s status=%s time=%.17g tf32_time=%.17g "
                     "orth_time=%.17g rayleigh_time=%.17g "
                     "fp64_time=%.17g final_time=%.17g back_time=%.17g "
                     "abs_residual=%.17g "
