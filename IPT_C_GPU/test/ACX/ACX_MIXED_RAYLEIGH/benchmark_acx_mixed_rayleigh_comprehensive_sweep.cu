...
static void load_config(Config *cfg)
{
    ...
    if (result_csv != NULL && result_csv[0] != '\0') {
        ...
        snprintf(cfg->result_csv, sizeof(cfg->result_csv),
                 "%s/acx_mixed_rayleigh_comprehensive_sweep.csv",
                 cfg->results_dir);
    }
    ...
    if (summary != NULL && summary[0] != '\0') {
        ...
        snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                 "%s/acx_mixed_rayleigh_comprehensive_sweep_summary.txt",
                 cfg->results_dir);
    }
}
...
int main(void)
{
    ...
    snprintf(summary_line, sizeof(summary_line),
             "started_at=%s methods=cusolver_syevd,original_ipt,"
             "original_acx,acx_mixed_tf32_fp64,"
             "acx_mixed_rayleigh_tf32_fp64 repeats=%d tf32_tol=%.17g "
             "fp64_tol=%.17g gpu=%s",
             time_buf, cfg.repeats, cfg.tf32_tol, cfg.fp64_tol, gpu_name);
    append_summary(&cfg, summary_line);

    printf("===== ACX mixed Rayleigh comprehensive sweep =====\n");
    printf("root=%s\nresults=%s\nlogs=%s\ncsv=%s\nsummary=%s\n", cfg.root,
    ...
    for (int eps_idx = 0; eps_idx < cfg.eps_count; ++eps_idx) {
        ...
        for (int n_idx = 0; n_idx < cfg.n_count; ++n_idx) {
            ...
            MethodResult ipt_result;
            MethodResult acx_result;
            MethodResult mixed_result;
            MethodResult rayleigh_result;
            char error[ERROR_LEN] = "";
            double matrix_start = 0.0;

            method_result_reset(&cusolver_result, "not_run");
            method_result_reset(&ipt_result, "not_run");
            method_result_reset(&acx_result, "not_run");
            method_result_reset(&mixed_result, "not_run");
            method_result_reset(&rayleigh_result, "not_run");
            ...
            if (make_host_matrix(&h_matrix, n, epsilon,
                ...
                snprintf(mixed_result.status, sizeof(mixed_result.status),
                         "failed");
                snprintf(rayleigh_result.status,
                         sizeof(rayleigh_result.status), "failed");
                ...
                snprintf(mixed_result.error, sizeof(mixed_result.error), "%s",
                         error);
                snprintf(rayleigh_result.error,
                         sizeof(rayleigh_result.error), "%s", error);
                failures += 5;
                goto row_done;
            }
            ...
            ipt_result = run_average_device("original IPT",
                                            run_original_ipt_once, blas_handle,
                                            d_matrix, n, &cfg);
            if (strcmp(ipt_result.status, "ok") != 0) {
                failures++;
            }
            acx_result = run_average_device("original ACX",
                                            run_original_acx_once, blas_handle,
                                            d_matrix, n, &cfg);
            if (strcmp(acx_result.status, "ok") != 0) {
                failures++;
            }
            mixed_result = run_average_device("ACX mixed TF32->FP64",
                                              run_acx_mixed_once, blas_handle,
                                              d_matrix, n, &cfg);
            if (strcmp(mixed_result.status, "ok") != 0) {
                failures++;
            }
            rayleigh_result = run_average_device(
        ...
        row_done:
            append_csv_row(&cfg, n, epsilon, "cusolver_syevd",
                           &cusolver_result, gpu_name);
            append_csv_row(&cfg, n, epsilon, "original_ipt_gpu_fp64",
                           &ipt_result, gpu_name);
            append_csv_row(&cfg, n, epsilon, "original_acx_gpu_fp64",
                           &acx_result, gpu_name);
            append_csv_row(&cfg, n, epsilon, "acx_mixed_tf32_fp64",
                           &mixed_result, gpu_name);
            append_csv_row(&cfg, n, epsilon,
                           "acx_mixed_rayleigh_tf32_fp64", &rayleigh_result,
                           gpu_name);

            wall_time_string(time_buf, sizeof(time_buf));
            snprintf(summary_line, sizeof(summary_line),
                     "epsilon=%.12g N=%d finished_at=%s "
                     "cusolver=%.17g ipt=%.17g acx=%.17g "
                     "mixed=%.17g rayleigh=%.17g "
                     "ipt_f_calls=%d acx_f_calls=%d mixed_f_calls=%d "
                     "rayleigh_f_calls=%d csv=%s",
                     epsilon, n, time_buf, cusolver_result.total_time_sec,
                     ipt_result.total_time_sec, acx_result.total_time_sec,
                     mixed_result.total_time_sec,
                     rayleigh_result.total_time_sec,
                     ipt_result.f_calls_total, acx_result.f_calls_total,
                     mixed_result.f_calls_total,
                     rayleigh_result.f_calls_total, cfg.result_csv);
            append_summary(&cfg, summary_line);
