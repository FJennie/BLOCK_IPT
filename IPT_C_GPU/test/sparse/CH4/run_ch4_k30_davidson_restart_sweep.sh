#!/usr/bin/env bash
#SBATCH --job-name=ch4_dav_restart
...
#SBATCH --output=/fs1/home/nudt_liujie/ftt/IPT_C_GPU/logs/sparse/CH4_K30_dubug/davidson_restart_sweep_%j.out
#SBATCH --error=/fs1/home/nudt_liujie/ftt/IPT_C_GPU/logs/sparse/CH4_K30_dubug/davidson_restart_sweep_%j.err

set -u -o pipefail

ROOT="${IPT_C_ROOT:-/fs1/home/nudt_liujie/ftt/IPT_C_GPU}"
RESULTS_BASE="${IPT_DAVIDSON_RESULTS_BASE:-$ROOT/results/sparse/CH4_K30_debug/davidson_restart_sweep}"
LOGS_BASE="${IPT_DAVIDSON_LOGS_BASE:-$ROOT/logs/sparse/CH4_K30_dubug/davidson_restart_sweep}"
CACHE="${IPT_CH4_MATRIX_CACHE:-$ROOT/results/sparse/CH4_K30_debug/ch4_sto6g_fci_15876_csc.bin}"
STEPS_LIST="${IPT_DAVIDSON_STEPS_LIST:-30 40 60 80}"
CLIP_LIST="${IPT_DAVIDSON_CLIP_LIST:-3e-9 1e-8 3e-8}"
RUNNER="$ROOT/test/sparse/CH4/run_ch4_sto6g_fci_15876_ipt.sh"
SWEEP_CSV="$RESULTS_BASE/davidson_restart_sweep.csv"
BEST_TXT="$RESULTS_BASE/davidson_restart_best.txt"

mkdir -p "$RESULTS_BASE" "$LOGS_BASE"
printf '%s\n' \
    "steps,denom_clip,exit_code,status,basis_cols,accepted_entries,restart_count,max_relative_eigen_residual,max_relative_eigen_residual_index,pair_28_residual,pair_29_residual,max_pair_0_28_residual,orthogonality_error,result_dir" \
    >"$SWEEP_CSV"

for steps in $STEPS_LIST; do
    for denom_clip in $CLIP_LIST; do
        case_name="steps_${steps}_clip_${denom_clip}_active_2"
        ...
        if [[ -s "$trial_csv" && -s "$pair_csv" && -s "$history_csv" ]]; then
            ...
            printf '%s\n' \
                "$steps,$denom_clip,$exit_code,$status,$basis_cols,$accepted_entries,$restart_count,$maximum,$maximum_index,$pair_28,$pair_29,$max_0_28,$orthogonality,$result_dir" \
                >>"$SWEEP_CSV"
        else
            printf '%s\n' \
                "$steps,$denom_clip,$exit_code,missing,0,0,0,nan,-1,nan,nan,nan,nan,$result_dir" \
                >>"$SWEEP_CSV"
        fi
    done
done
