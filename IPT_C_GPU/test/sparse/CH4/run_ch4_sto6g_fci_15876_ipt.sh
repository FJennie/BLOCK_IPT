...
export IPT_CUDA_ARCH="${IPT_CUDA_ARCH:-sm_80}"
...
echo "IPT_DAVIDSON_DENOM_CLIP=auto"
echo "IPT_DAVIDSON_TRIAL_ACCEPT=valid_basis"
echo "IPT_DAVIDSON_ORTHO_REPEATS=2"
echo "IPT_DAVIDSON_BEST_SO_FAR=enabled"
echo "IPT_DAVIDSON_SOFT_LOCKING=residual_le_converged_tol"
echo "IPT_DAVIDSON_RESTART=thick_pre_dimension_maxKPlusMax16TwoThirdK_minK_lastBlock"
...
if [ "${IPT_SKIP_BUILD:-0}" = "1" ]; then
    ...
    set -x
    nvcc -O3 -std=c++14 -lineinfo -arch="$IPT_CUDA_ARCH"     -I"$PRIMME_ROOT/include"     -I"$PYTHON_ROOT/include/python3.8"     "$TEST_DIR/benchmark_ch4_sto6g_fci_15876_ipt.cu"     "$PRIMME_ROOT/lib/libprimme.a"     -o "$BENCH_BIN"     -lcublas -lcusparse -lcusolver -lcudart     -L"$PYTHON_ROOT/lib" -lpython3.8     -L"$TEST_DIR" -lopenblas     -lpthread -ldl -lutil -lrt     -Xlinker -rpath -Xlinker "$GCC_LIB_DIR"     -Xlinker -rpath -Xlinker "$PYTHON_ROOT/lib"     -Xlinker -rpath -Xlinker "$OPENBLAS_DIR"     -lgfortran "$GCC_LIB_DIR/libstdc++.so" -lm
    set +x
fi
echo "===== run ====="
"$BENCH_BIN"
