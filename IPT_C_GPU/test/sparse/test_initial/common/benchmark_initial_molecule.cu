...
static const char *INITIAL_MOLECULE_PYTHON = R"PY(
def build_initial_molecule_integrals():
    import os
    import numpy as np
    ...
    coordinates = os.environ["IPT_MOLECULE_ATOMS"].replace("|", "\n")
    basis = os.environ.get("IPT_MOLECULE_BASIS", "sto-6g")
    charge = int(os.environ.get("IPT_MOLECULE_CHARGE", "0"))
    spin = int(os.environ.get("IPT_MOLECULE_SPIN", "0"))
    active_orbitals = int(os.environ.get("IPT_ACTIVE_ORBITALS", "0"))
    active_electrons = int(os.environ.get("IPT_ACTIVE_ELECTRONS", "0"))

    mol = gto.M(
        atom=coordinates, basis=basis, charge=charge, spin=spin,
        symmetry=False, verbose=0
    )
    hf = scf.RHF(mol) if spin == 0 else scf.ROHF(mol)
    hf.verbose = 0
    hf.kernel()
    if not hf.converged:
        raise RuntimeError("PySCF SCF did not converge")

    full_nelec = mol.nelectron
    full_norb = hf.mo_coeff.shape[1]
    if active_orbitals > 0 or active_electrons > 0:
        if active_orbitals <= 0 or active_electrons <= 0:
            raise RuntimeError("CAS requires both IPT_ACTIVE_ORBITALS and IPT_ACTIVE_ELECTRONS")
        if active_electrons > full_nelec or active_orbitals > full_norb:
            raise RuntimeError("invalid CAS larger than full space")
        ncore = (full_nelec - active_electrons) // 2
        if ncore < 0 or ncore + active_orbitals > full_norb:
            raise RuntimeError("invalid closed-shell CAS core/orbital window")
        cas = mcscf.CASCI(hf, active_orbitals, active_electrons)
        h1eff, ecore = cas.get_h1eff(hf.mo_coeff)
        mo_active = hf.mo_coeff[:, ncore:ncore + active_orbitals]
        eri = ao2mo.kernel(mol, mo_active, compact=False)
        norb = active_orbitals
        nelec = active_electrons
    else:
        nelec = full_nelec
        norb = full_norb
...
static const char *matrix_id(void)
{
    const char *id = getenv("IPT_MATRIX_ID");

    return (id == NULL || id[0] == '\0') ? "initial_molecule" : id;
}
...
static bool parse_integral_tuple(PyObject *tuple, ActiveSpaceData *data)
{
    ...
    if (data->norb <= 0 || data->active_electrons <= 0 ||
        data->neleca < 0 || data->nelecb < 0 || data->na <= 0 ||
        data->nb <= 0 ||
        data->h1.size() != (size_t)data->norb * (size_t)data->norb ||
        data->eri.size() != (size_t)data->norb * (size_t)data->norb *
                               (size_t)data->norb * (size_t)data->norb ||
        (long long)data->na * (long long)data->nb >
            (long long)std::numeric_limits<int>::max()) {
        fprintf(stderr,
                "unexpected molecular active space: norb=%d "
...
static bool generate_active_space_integrals(ActiveSpaceData *data)
{
    ...
    printf("Generating molecular FCI/CAS integrals with PySCF...\n");
    ...
    PyObject *module_result =
        PyRun_String(INITIAL_MOLECULE_PYTHON, Py_file_input, globals,
                     globals);
    ...
    PyObject *result = PyRun_String(
        "build_initial_molecule_integrals()", Py_eval_input, globals,
        globals);
...
{
    const char magic[8] = {'I', 'P', 'T', 'G', 'E', 'N', 'C', '1'};
...
int main(void)
{
    ...
    const char *id = matrix_id();
    const char *cache_path = getenv("IPT_MATRIX_CACHE");
    ...
    if (load_matrix &&
        ...
        matrix_source = "cache";
        printf("Loaded molecular CSC matrix cache from %s\n", cache_path);
    } else {
        if (load_matrix) {
            fprintf(stderr,
                    "molecular matrix cache unavailable or invalid; generating "
                    "matrix: %s\n",
                    cache_path == NULL ? "(unset)" : cache_path);
        }
        if (!generate_active_space_integrals(&active)) {
            fprintf(stderr,
                    "failed to generate molecular FCI/CAS active "
                    "integrals\n");
            return 1;
        }
        if (!generate_hamiltonian_csc(active, &owned_matrix)) {
            fprintf(stderr,
                    "failed to generate molecular FCI/CAS CSC matrix\n");
            return 1;
        }
        {
        ...
        if (save_matrix) {
            if (!write_ch4_matrix_cache(cache_path, owned_matrix, matrix_norm,
                                        cached_diagonal, active)) {
                fprintf(stderr, "failed to save molecular matrix cache to %s\n",
                        cache_path == NULL ? "(unset)" : cache_path);
                return 1;
            }
            printf("Saved molecular CSC matrix cache to %s\n", cache_path);
        }
    }
    ...
    fprintf(summary,
            ...
            "molecule_charge=%s\n"
            "molecule_spin=%s\n"
            "active_orbitals_env=%s\n"
            "active_electrons_env=%s\n"
            "active_orbitals=%d\n"
            "active_electrons=%d\n"
            "matrix_source=%s\nn=%d\nnnz=%d\nmatrix_inf_norm=%.17g\n"
            "matrix_cache_path=%s\n"
            ...
            id,
            getenv("IPT_MOLECULE_ATOMS") == NULL
                ? "(unset)"
                : getenv("IPT_MOLECULE_ATOMS"),
            getenv("IPT_MOLECULE_BASIS") == NULL
                ? "(unset)"
                : getenv("IPT_MOLECULE_BASIS"),
            getenv("IPT_MOLECULE_CHARGE") == NULL
                ? "0"
                : getenv("IPT_MOLECULE_CHARGE"),
            getenv("IPT_MOLECULE_SPIN") == NULL
                ? "0"
                : getenv("IPT_MOLECULE_SPIN"),
            getenv("IPT_ACTIVE_ORBITALS") == NULL
                ? "0"
                : getenv("IPT_ACTIVE_ORBITALS"),
            getenv("IPT_ACTIVE_ELECTRONS") == NULL
                ? "0"
                : getenv("IPT_ACTIVE_ELECTRONS"),
            active.norb, active.active_electrons, matrix_source, matrix.n,
            matrix.nnz, matrix_norm,
