...
static const char *NH3_STO6G_FCI_PYTHON = R"PY(
def build_nh3_sto6g_full_fci_integrals():
    import numpy as np
...
"""
    ...
    hf.kernel()
    if not hf.converged:
        raise RuntimeError("PySCF RHF did not converge for NH3/STO-6G full FCI")
...
static bool parse_integral_tuple(PyObject *tuple, ActiveSpaceData *data)
{
    ...
    if (data->norb != 8 || data->active_electrons != 10 ||
        data->neleca != 5 || data->nelecb != 5 || data->na != 56 ||
        data->nb != 56 || data->h1.size() != 64U ||
        data->eri.size() != 4096U) {
        fprintf(stderr,
                "unexpected NH3/STO-6G full FCI active space: norb=%d "
...
static bool generate_active_space_integrals(ActiveSpaceData *data)
{
    ...
    printf("Generating NH3/STO-6G full FCI integrals with "
           "PySCF...\n");
    ...
    PyObject *module_result =
        PyRun_String(NH3_STO6G_FCI_PYTHON, Py_file_input, globals, globals);
    ...
    PyObject *result = PyRun_String(
        "build_nh3_sto6g_full_fci_integrals()", Py_eval_input, globals,
        globals);
...
static void print_sorted_diagonal_debug(const CscMatrixView *matrix, int k,
                                        double threshold)
{
    if (!env_bool("IPT_DEBUG_PREPARATION", 0)) {
        return;
    }

    std::vector<double> diagonal((size_t)matrix->n, 0.0);
    std::vector<int> perm((size_t)matrix->n, 0);

    for (int col = 0; col < matrix->n; ++col) {
        perm[(size_t)col] = col;
        for (int p = matrix->col_ptr[col]; p < matrix->col_ptr[col + 1];
             ++p) {
            if (matrix->row_ind[p] == col) {
                diagonal[(size_t)col] = matrix->values[p];
                break;
            }
        }
    }
    ...
    std::stable_sort(perm.begin(), perm.end(),
                     [&](int a, int b) {
                         if (diagonal[(size_t)a] < diagonal[(size_t)b]) {
                             return true;
                         }
                         if (diagonal[(size_t)b] < diagonal[(size_t)a]) {
                             return false;
                         }
                         return a < b;
                     });

    int count = std::min(matrix->n, 10);
    printf("[NH3 benchmark] sorted diagonal first %d entries "
           "(requested k=%d threshold=%.17g)\n",
           count, k, threshold);
    for (int rank = 0; rank < count; ++rank) {
        int original_index = perm[(size_t)rank];
        double value = diagonal[(size_t)original_index];

        if (rank + 1 < count) {
            int next_index = perm[(size_t)(rank + 1)];
            double gap = diagonal[(size_t)next_index] - value;
            printf("[NH3 benchmark] rank=%d original_index=%d "
                   "diagonal=%.17g gap_to_next=%.17g\n",
                   rank, original_index, value, gap);
        } else {
            printf("[NH3 benchmark] rank=%d original_index=%d "
                   "diagonal=%.17g\n",
                                ...
                                int repeat)
{
    ...
    if (status != IPT_CUDA_SUCCESS) {
        ...
        result.fixed_point_residual = ipt.fixed_point_residual;
...
static void write_aggregate(FILE *csv, const char *method,
                            const std::vector<TrialResult> &results,
                            double eigenvalue_abs_residual, double tol,
                            int maxiter, int k)
{
    const TrialResult *last = results.empty() ? NULL : &results.back();

    fprintf(csv,
            "%s,%d,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,"
            "%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%d,%s,%.17g,%d\n",
            method, k, average(results, &TrialResult::time_sec),
            average(results, &TrialResult::api_total_time_sec),
            average(results, &TrialResult::preparation_time_sec),
            average(results, &TrialResult::transfer_setup_time_sec),
            average(results, &TrialResult::iteration_time_sec),
            average(results, &TrialResult::rayleigh_ritz_time_sec),
            average_int(results, &TrialResult::iterations),
            average_ll(results, &TrialResult::matvecs),
            last != NULL ? last->eigenvalue : NAN,
            last != NULL ? last->residual : NAN,
            last != NULL ? last->fixed_point_residual : NAN,
            eigenvalue_abs_residual, last != NULL ? last->status : -1,
            last != NULL ? last->error : "no_results", tol, maxiter);
    fflush(csv);
}

static void write_summary(FILE *summary, const char *method,
                          const std::vector<TrialResult> &results,
                          double eigenvalue_abs_residual)
{
    ...
    fprintf(summary,
            ...
            "avg_matvecs=%.17g avg_first_eigenvalue=%.17g "
            "last_first_eigenvalue=%.17g last_max_residual=%.17g "
            "last_relative_fixed_point_residual=%.17g "
            "eigenvalue_abs_residual=%.17g avg_api_total_time_sec=%.17g "
            "avg_preparation_time_sec=%.17g avg_transfer_setup_time_sec=%.17g "
            "avg_iteration_time_sec=%.17g avg_rayleigh_ritz_time_sec=%.17g\n",
...
int main(void)
{
    ...
    double degeneracy_threshold =
        env_double("IPT_DEGENERACY_THRESHOLD", 0.0);
    ...
    snprintf(csv_path, sizeof(csv_path),
             "%s/nh3_sto6g_fci_3136_ipt_trials.csv", dir);
    snprintf(summary_path, sizeof(summary_path),
             "%s/nh3_sto6g_fci_3136_ipt_summary.txt", dir);
    ...
    fprintf(csv,
            "method,k,avg_solve_time_sec,avg_api_total_time_sec,"
            "avg_preparation_time_sec,avg_transfer_setup_time_sec,"
            "avg_iteration_time_sec,avg_rayleigh_ritz_time_sec,"
            "avg_iterations,avg_matvecs,last_first_eigenvalue,"
            "last_max_relative_residual,last_relative_fixed_point_residual,"
            "eigenvalue_abs_residual,status,error,tol,maxiter\n");

    printf("NH3/STO-6G full FCI CSC benchmark: n=%d "
    ...
    print_sorted_diagonal_debug(&matrix, ipt_k, degeneracy_threshold);
    fflush(stdout);

    if (run_warmup) {
        fprintf(summary,
                "warmup enabled RUN_WARMUP=1; warmup trials are not "
                "written to CSV\n\n");
        printf("warmup start\n");
        fflush(stdout);
        {
            ...
            printf("warmup IPT_GPU_BLOCK k=%d solve=%.6e preparation=%.6e "
                   "setup=%.6e iteration=%.6e rr=%.6e api_total=%.6e "
                   "fp_residual=%.3e max_residual=%.3e "
                   "first_eigenvalue=%.15g status=%d\n",
                   ipt_k, warmup_ipt.time_sec,
                   ...
                   warmup_ipt.fixed_point_residual, warmup_ipt.residual,
                   warmup_ipt.eigenvalue, warmup_ipt.status);
        }
        if (run_primme) {
            TrialResult warmup_primme =
                run_primme_once(&matrix, tol, primme_max_matvecs, matrix_norm,
                                ipt_k, 0);
            printf("warmup PRIMME_CUBLAS k=%d solve=%.6e setup=%.6e "
                   "max_residual=%.3e first_eigenvalue=%.15g status=%d\n",
                   ipt_k, warmup_primme.time_sec,
                   warmup_primme.transfer_setup_time_sec,
                   warmup_primme.residual, warmup_primme.eigenvalue,
                   warmup_primme.status);
        }
        printf("warmup done\n");
        fflush(stdout);
    } else {
        fprintf(summary, "warmup skipped RUN_WARMUP=0\n\n");
    }
    ...
    for (int repeat = 1; repeat <= repeats; ++repeat) {
        ...
        printf("repeat %d IPT_GPU_BLOCK k=%d solve=%.6e preparation=%.6e "
               "setup=%.6e iteration=%.6e rr=%.6e api_total=%.6e "
               "fp_residual=%.3e max_residual=%.3e "
               "first_eigenvalue=%.15g\n",
               repeat, ipt_k, ipt_result.time_sec,
               ...
               ipt_result.fixed_point_residual, ipt_result.residual,
               ipt_result.eigenvalue);
        fflush(stdout);
        ...
        if (run_primme) {
            ...
            printf("repeat %d PRIMME_CUBLAS k=%d solve=%.6e setup=%.6e "
                   "max_residual=%.3e first_eigenvalue=%.15g\n",
                   repeat, ipt_k, primme_result.time_sec,
                   primme_result.transfer_setup_time_sec,
                   primme_result.residual, primme_result.eigenvalue);
        }
        fflush(stdout);
    }

    if (run_primme && !primme_results.empty()) {
        reference_eigenvalue = primme_results.back().eigenvalue;
    }
    if (!ipt_results.empty() && isfinite(reference_eigenvalue)) {
        ipt_eigenvalue_abs_residual =
            fabs(ipt_results.back().eigenvalue - reference_eigenvalue);
    }

    if (run_primme) {
        write_aggregate(csv, "PRIMME_CUBLAS", primme_results, 0.0, tol,
                        (int)primme_max_matvecs, ipt_k);
    }
    write_aggregate(csv, "IPT_CUDA_BLOCK_CLUSTER", ipt_results,
                    ipt_eigenvalue_abs_residual, tol, ipt_maxiter, ipt_k);

    fprintf(summary,
            "matrix=nh3_sto6g_fci\nn=%d\nnnz=%d\n"
            "k=%d\nrepeats=%d\ntol=%.17g\nipt_maxiter=%d\n"
    ...
    if (run_primme) {
        write_summary(summary, "PRIMME_CUBLAS", primme_results, 0.0);
    }
    write_summary(summary, "IPT_CUDA_BLOCK_CLUSTER", ipt_results,
                  ipt_eigenvalue_abs_residual);
