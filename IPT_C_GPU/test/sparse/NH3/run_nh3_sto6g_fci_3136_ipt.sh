...
RUN_LOG="$LOG_DIR/nh3_sto6g_fci_3136_ipt_${SLURM_JOB_ID:-manual}.log"
ln -sfn "$RUN_LOG" "$LOG_DIR/latest_nh3_sto6g_fci_3136_ipt.log"
...
export IPT_BLOCK_CLUSTER_MAXITER="${IPT_BLOCK_CLUSTER_MAXITER:-100}"
export IPT_BLOCK_CLUSTER_TOL="${IPT_BLOCK_CLUSTER_TOL:-$IPT_TOL}"
export IPT_BLOCK_CLUSTER_DAMPING="${IPT_BLOCK_CLUSTER_DAMPING:-1}"
export PRIMME_MAX_MATVECS="${PRIMME_MAX_MATVECS:-5000}"
...
echo "===== NH3/STO-6G full FCI IPT sparse benchmark ====="
date; hostname
...
echo "IPT_K=$IPT_K"
echo "IPT_DEBUG_PREPARATION=$IPT_DEBUG_PREPARATION"
echo "IPT_DEGENERACY_THRESHOLD=$IPT_DEGENERACY_THRESHOLD"
...
echo "IPT_BLOCK_CLUSTER_DAMPING=$IPT_BLOCK_CLUSTER_DAMPING"
...
echo "===== build ====="
set -x
nvcc -O3 -std=c++14 -lineinfo     -gencode arch=compute_70,code=sm_70     -gencode arch=compute_80,code=sm_80     -I"$PRIMME_ROOT/include"     -I"$PYTHON_ROOT/include/python3.8"     "$TEST_DIR/benchmark_nh3_sto6g_fci_3136_ipt.cu"     "$PRIMME_ROOT/lib/libprimme.a"     -o "$TEST_DIR/benchmark_nh3_sto6g_fci_3136_ipt_cuda"     -lcublas -lcusparse -lcusolver -lcudart     -L"$PYTHON_ROOT/lib" -lpython3.8     -L"$TEST_DIR" -lopenblas     -lpthread -ldl -lutil -lrt     -Xlinker -rpath -Xlinker "$GCC_LIB_DIR"     -Xlinker -rpath -Xlinker "$PYTHON_ROOT/lib"     -Xlinker -rpath -Xlinker "$OPENBLAS_DIR"     -lgfortran "$GCC_LIB_DIR/libstdc++.so" -lm
set +x
echo "===== run ====="
"$TEST_DIR/benchmark_nh3_sto6g_fci_3136_ipt_cuda"
status=$?
echo "===== output files ====="
