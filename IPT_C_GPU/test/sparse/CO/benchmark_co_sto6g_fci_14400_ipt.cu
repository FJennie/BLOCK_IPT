...
static const char *CO_STO6G_FCI_PYTHON = R"PY(
def build_co_sto6g_full_fci_integrals():
    import numpy as np
    ...
    coordinates = """
C 0.000000 0.000000 -0.564100;
O 0.000000 0.000000 0.564100
"""
    ...
    hf.kernel()
    if not hf.converged:
        raise RuntimeError("PySCF RHF did not converge for CO/STO-6G full FCI")
...
static bool parse_integral_tuple(PyObject *tuple, ActiveSpaceData *data)
{
    ...
    if (data->norb != 10 || data->active_electrons != 14 ||
        data->neleca != 7 || data->nelecb != 7 || data->na != 120 ||
        data->nb != 120 || data->h1.size() != 100U ||
        data->eri.size() != 10000U) {
        fprintf(stderr,
                "unexpected CO/STO-6G full FCI active space: norb=%d "
...
static bool generate_active_space_integrals(ActiveSpaceData *data)
{
    ...
    printf("Generating CO/STO-6G full FCI integrals with "
           "PySCF...\n");
    ...
    PyObject *module_result =
        PyRun_String(CO_STO6G_FCI_PYTHON, Py_file_input, globals, globals);
    ...
    PyObject *result = PyRun_String(
        "build_co_sto6g_full_fci_integrals()", Py_eval_input, globals,
        globals);
                                ...
                                const double *vector, double eigenvalue)
{
    ...
    {
        double denom = std::max(1.0, fabs(eigenvalue)) *
                                   ...
                                   int repeat)
{
    ...
    double eval = 0.0;
    double rnorm = 0.0;
    std::vector<double> evec((size_t)matrix->n, 0.0);
    int ret = 0;
    ...
    {
        double start = now_seconds();
        ret = dprimme(&eval, evec.data(), &rnorm, &primme);
    ...
    if (ret != 0) {
        ...
        result.residual = relative_residual(matrix, evec.data(), eval);
    }
...
static TrialResult run_ipt_once(const CscMatrixView *matrix, double tol,
                                int maxiter, int repeat)
{
    TrialResult result = {"IPT_CUDA_SPARSE", repeat, NAN, 0, 0, NAN, NAN, 0,
                          ""};
    IPTCudaResult ipt = {0, 0, 0, NULL, NULL};
    int status = IPT_CUDA_SUCCESS;
    double api_time_sec = NAN;

    {
        double start = now_seconds();
        status = ipt_cuda_sparse_csc_tol(matrix->col_ptr, matrix->row_ind,
                                         matrix->values, matrix->n, 1,
                                         matrix->nnz, tol, maxiter, &ipt);
        cudaDeviceSynchronize();
        api_time_sec = now_seconds() - start;
        result.time_sec =
            (isfinite(ipt.solve_time_sec) && ipt.solve_time_sec > 0.0)
                ? ipt.solve_time_sec
                : api_time_sec;
    }

    printf("repeat %d IPT internal timing: compute=%.6e api_total=%.6e "
           "preparation=%.6e transfer_setup=%.6e iteration=%.6e "
           "rayleigh=%.6e fixed_point=%.3e\n",
           repeat, result.time_sec, api_time_sec, ipt.preparation_time_sec,
           ipt.transfer_setup_time_sec, ipt.iteration_time_sec,
           ipt.rayleigh_ritz_time_sec, ipt.fixed_point_residual);
    fflush(stdout);
    ...
    if (status != IPT_CUDA_SUCCESS) {
        ...
        result.matvecs = ipt.matvecs > 0 ? ipt.matvecs : (long long)ipt.iterations;
        result.eigenvalue = ipt.values[0];
        result.residual =
            relative_residual(matrix, ipt.vectors, result.eigenvalue);
    }
...
{
    fprintf(csv,
            "%s,%d,%.17g,%d,%lld,%.17g,%.17g,%d,%s,%.17g,%d\n",
            result.method, result.repeat, result.time_sec, result.iterations,
            result.matvecs, result.eigenvalue, result.residual, result.status,
            result.error, tol, maxiter);
    fflush(csv);
}
...
int main(void)
{
    ...
    if (!generate_active_space_integrals(&active)) {
        fprintf(stderr,
                "failed to generate CO/STO-6G full FCI active integrals\n");
        return 1;
    }

    if (!generate_hamiltonian_csc(active, &owned_matrix)) {
        fprintf(stderr,
                "failed to generate CO/STO-6G full FCI CSC matrix\n");
        return 1;
    }
    ...
    snprintf(csv_path, sizeof(csv_path),
             "%s/co_sto6g_fci_14400_ipt_trials.csv", dir);
    snprintf(summary_path, sizeof(summary_path),
             "%s/co_sto6g_fci_14400_ipt_summary.txt", dir);
    ...
    fprintf(csv,
            "method,repeat,time_sec,iterations,matvecs,eigenvalue,"
            "relative_residual,status,error,tol,maxiter\n");

    printf("CO/STO-6G full FCI FCI CSC benchmark: n=%d "
           "nnz=%d repeats=%d tol=%.3e run_primme=%d\n",
           matrix.n, matrix.nnz, repeats, tol, run_primme);
    ...
    fprintf(summary,
            "timing_note=IPT_CUDA_SPARSE time_sec is internal compute time "
            "solve_time_sec=iteration_time_sec+rayleigh_ritz_time_sec; "
            "it excludes preparation, transfer/setup, final unpermute, and "
            "host result copies. PRIMME time_sec remains the dprimme call "
            "time.\n\n");
    ...
    for (int repeat = 1; repeat <= repeats; ++repeat) {
        if (run_primme) {
            ...
            write_trial(csv, primme_result, tol, (int)primme_max_matvecs);
        }

        TrialResult ipt_result =
            run_ipt_once(&matrix, tol, ipt_maxiter, repeat);
        ipt_results.push_back(ipt_result);
        write_trial(csv, ipt_result, tol, ipt_maxiter);

        printf("repeat %d IPT time=%.6e residual=%.3e eigenvalue=%.15g\n",
               repeat, ipt_result.time_sec, ipt_result.residual,
    ...
    fprintf(summary,
            ...
            matrix.n, matrix.nnz, repeats, tol, ipt_maxiter, run_primme,
