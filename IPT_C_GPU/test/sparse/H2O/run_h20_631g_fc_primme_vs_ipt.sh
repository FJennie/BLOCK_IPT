...
RUN_LOG="$LOG_DIR/h2o_631g_fc_fci_245025_primme_vs_ipt_${SLURM_JOB_ID:-manual}.log"
ln -sfn "$RUN_LOG" "$LOG_DIR/latest_h2o_631g_fc_fci_245025_primme_vs_ipt.log"
...
export IPT_TOL="${IPT_TOL:-1e-10}"
export IPT_MAXITER="${IPT_MAXITER:-200}"
export PRIMME_MAX_MATVECS="${PRIMME_MAX_MATVECS:-300}"
export RUN_PRIMME="${RUN_PRIMME:-0}"
export IPT_EST_NNZ_PER_COL="${IPT_EST_NNZ_PER_COL:-1500}"
...
echo "===== H2O/6-31G frozen-core CAS(8e,12o) PRIMME vs IPT sparse benchmark ====="
date
hostname
...
nvcc -O3 -std=c++14 -lineinfo -arch=sm_80 \
    ...
    "$TEST_DIR/benchmark_h20_631g_fc_primme_vs_ipt.cu" \
    "$PRIMME_ROOT/lib/libprimme.a" \
    -o "$TEST_DIR/benchmark_h20_631g_fc_primme_vs_ipt_cuda" \
...
echo
echo "===== run ====="
"$TEST_DIR/benchmark_h20_631g_fc_primme_vs_ipt_cuda"
status=$?
