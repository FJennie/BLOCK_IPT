...
static const char *H2O_631G_CAS10E10O_PYTHON = R"PY(
def build_h2o_631g_cas10e10o_integrals():
    import numpy as np
...
static bool parse_integral_tuple(PyObject *tuple, ActiveSpaceData *data)
{
    ...
    if (data->norb != 10 || data->active_electrons != 10 ||
        data->neleca != 5 || data->nelecb != 5 || data->na != 252 ||
        data->nb != 252 || data->h1.size() != 100U ||
        data->eri.size() != 10000U) {
        fprintf(stderr,
                "unexpected H2O/6-31G CAS(10e,10o) active space: norb=%d "
...
static bool generate_active_space_integrals(ActiveSpaceData *data)
{
    ...
    printf("Generating H2O/6-31G CAS(10e,10o) integrals with "
           "PySCF...\n");
    ...
    PyObject *module_result =
        PyRun_String(H2O_631G_CAS10E10O_PYTHON, Py_file_input, globals, globals);
    ...
    PyObject *result = PyRun_String(
        "build_h2o_631g_cas10e10o_integrals()", Py_eval_input, globals,
        globals);
...
int main(void)
{
    ...
    if (!generate_active_space_integrals(&active)) {
        fprintf(stderr,
                "failed to generate H2O/6-31G CAS(10e,10o) active integrals\n");
        return 1;
    }

    if (!generate_hamiltonian_csc(active, &owned_matrix)) {
        fprintf(stderr,
                "failed to generate H2O/6-31G CAS(10e,10o) CSC matrix\n");
        return 1;
    }
    ...
    snprintf(csv_path, sizeof(csv_path),
             "%s/h2o_631g_cas10e10o_fci_63504_primme_vs_ipt_trials.csv", dir);
    snprintf(summary_path, sizeof(summary_path),
             "%s/h2o_631g_cas10e10o_fci_63504_primme_vs_ipt_summary.txt", dir);
    ...
    printf("H2O/6-31G CAS(10e,10o) FCI CSC benchmark: n=%d "
