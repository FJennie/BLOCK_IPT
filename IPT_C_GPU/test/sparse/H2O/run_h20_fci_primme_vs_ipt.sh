...
TEST_DIR="$ROOT/test/sparse/H2O_FCI"

mkdir -p "$LOG_DIR" "$RESULTS_DIR" "$TEST_DIR"

RUN_LOG="$LOG_DIR/h2o_fci_pyscf_441_primme_vs_ipt_${SLURM_JOB_ID:-manual}.log"
ln -sfn "$RUN_LOG" "$LOG_DIR/latest_h2o_fci_pyscf_441_primme_vs_ipt.log"
...
echo "===== H2O/sto-6g FCI PySCF PRIMME vs IPT sparse benchmark ====="
date
hostname
...
echo "ROOT=$ROOT"
echo "PRIMME_ROOT=$PRIMME_ROOT"
echo "PYTHON_ROOT=$PYTHON_ROOT"
echo "PYSCF_SITE_PACKAGES=$PYSCF_SITE_PACKAGES"
...
nvcc -O3 -std=c++14 -lineinfo -arch=sm_80 \
    ...
    -o "$TEST_DIR/benchmark_h20_fci_primme_vs_ipt_cuda" \
...
echo
echo "===== run ====="
"$TEST_DIR/benchmark_h20_fci_primme_vs_ipt_cuda"
status=$?
