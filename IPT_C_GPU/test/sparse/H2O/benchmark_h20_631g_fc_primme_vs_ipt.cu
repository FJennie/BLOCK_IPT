...
#define DEFAULT_TOL 1.0e-10
#define DEFAULT_IPT_MAXITER 200
#define DEFAULT_PRIMME_MAX_MATVECS 300
...
static const char *H2O_631G_FC_PYTHON = R"PY(
def build_h2o_631g_frozen_core_integrals():
    import numpy as np
...
"""
    ...
    mol = gto.M(atom=coordinates, basis="6-31g", symmetry=True, verbose=0)
    hf = scf.RHF(mol)
    hf.verbose = 0
    hf.kernel()
    if not hf.converged:
        raise RuntimeError("PySCF RHF did not converge for H2O/6-31G")

    mc = mcscf.CASCI(hf, 12, 8)
    h1eff, ecore = mc.get_h1eff()
    eri = ao2mo.restore(1, mc.get_h2eff(), mc.ncas)

    neleca, nelecb = mc.nelecas
    na = fci.cistring.num_strings(mc.ncas, neleca)
    nb = fci.cistring.num_strings(mc.ncas, nelecb)
...
static bool parse_integral_tuple(PyObject *tuple, ActiveSpaceData *data)
{
    ...
    if (data->norb != 12 || data->active_electrons != 8 ||
        data->neleca != 4 || data->nelecb != 4 || data->na != 495 ||
        data->nb != 495 || data->h1.size() != 144U ||
        data->eri.size() != 20736U) {
        fprintf(stderr,
                "unexpected H2O/6-31G frozen-core active space: norb=%d "
...
static bool generate_active_space_integrals(ActiveSpaceData *data)
{
    ...
    printf("Generating H2O/6-31G frozen-core CAS(8e,12o) integrals with "
           "PySCF...\n");
    ...
    PyObject *module_result =
        PyRun_String(H2O_631G_FC_PYTHON, Py_file_input, globals, globals);
    ...
    PyObject *result = PyRun_String(
        "build_h2o_631g_frozen_core_integrals()", Py_eval_input, globals,
        globals);
                                ...
                                int maxiter, int repeat)
{
    ...
    if (status != IPT_CUDA_SUCCESS) {
        ...
        result.matvecs = (long long)ipt.iterations;
...
int main(void)
{
    ...
    if (!generate_active_space_integrals(&active)) {
        fprintf(stderr,
                "failed to generate H2O/6-31G frozen-core active integrals\n");
        return 1;
    }

    if (!generate_hamiltonian_csc(active, &owned_matrix)) {
        fprintf(stderr,
                "failed to generate H2O/6-31G frozen-core CSC matrix\n");
        return 1;
    }
    ...
    snprintf(csv_path, sizeof(csv_path),
             "%s/h2o_631g_fc_fci_245025_primme_vs_ipt_trials.csv", dir);
    snprintf(summary_path, sizeof(summary_path),
             "%s/h2o_631g_fc_fci_245025_primme_vs_ipt_summary.txt", dir);
    ...
    printf("H2O/6-31G frozen-core CAS(8e,12o) FCI CSC benchmark: n=%d "
    ...
    if (run_primme) {
        TrialResult warm_primme =
            run_primme_once(&matrix, tol, primme_max_matvecs, matrix_norm, 0);
        fprintf(summary,
        ...
        fprintf(summary, "warmup PRIMME skipped RUN_PRIMME=0\n");
    }

    {
        TrialResult warm_ipt = run_ipt_once(&matrix, tol, ipt_maxiter, 0);
        fprintf(summary,
    ...
    fprintf(summary,
            "matrix=h2o_631g_frozen_o1s_core_cas8e12o_fci\nn=%d\nnnz=%d\n"
            "k=1\nrepeats=%d\ntol=%.17g\nipt_maxiter=%d\n"
