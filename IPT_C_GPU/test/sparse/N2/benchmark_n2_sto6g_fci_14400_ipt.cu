...
static const char *N2_STO6G_FCI_PYTHON = R"PY(
def build_n2_sto6g_full_fci_integrals():
    import numpy as np
...
"""
    ...
    hf.kernel()
    if not hf.converged:
        raise RuntimeError("PySCF RHF did not converge for N2/STO-6G full FCI")
...
static bool parse_integral_tuple(PyObject *tuple, ActiveSpaceData *data)
{
    ...
    if (data->norb != 10 || data->active_electrons != 14 ||
        ...
        fprintf(stderr,
                "unexpected N2/STO-6G full FCI active space: norb=%d "
...
static bool generate_active_space_integrals(ActiveSpaceData *data)
{
    ...
    printf("Generating N2/STO-6G full FCI integrals with "
           "PySCF...\n");
    ...
    PyObject *module_result =
        PyRun_String(N2_STO6G_FCI_PYTHON, Py_file_input, globals, globals);
    ...
    PyObject *result = PyRun_String(
        "build_n2_sto6g_full_fci_integrals()", Py_eval_input, globals,
        globals);
...
int main(void)
{
    ...
    if (!generate_active_space_integrals(&active)) {
        fprintf(stderr,
                "failed to generate N2/STO-6G full FCI active integrals\n");
        return 1;
    }

    if (!generate_hamiltonian_csc(active, &owned_matrix)) {
        fprintf(stderr,
                "failed to generate N2/STO-6G full FCI CSC matrix\n");
        return 1;
    }
    ...
    snprintf(csv_path, sizeof(csv_path),
             "%s/n2_sto6g_fci_14400_ipt_trials.csv", dir);
    snprintf(summary_path, sizeof(summary_path),
             "%s/n2_sto6g_fci_14400_ipt_summary.txt", dir);
    ...
    printf("N2/STO-6G full FCI FCI CSC benchmark: n=%d "
