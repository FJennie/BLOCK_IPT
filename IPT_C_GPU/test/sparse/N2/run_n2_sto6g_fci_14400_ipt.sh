...
TEST_DIR="$ROOT/test/sparse/N2"
mkdir -p "$LOG_DIR" "$RESULTS_DIR" "$TEST_DIR"
RUN_LOG="$LOG_DIR/n2_sto6g_fci_14400_ipt_${SLURM_JOB_ID:-manual}.log"
ln -sfn "$RUN_LOG" "$LOG_DIR/latest_n2_sto6g_fci_14400_ipt.log"
...
echo "===== N2/STO-6G full FCI IPT sparse benchmark ====="
date; hostname
...
echo "===== build ====="
set -x
nvcc -O3 -std=c++14 -lineinfo -arch=sm_80     -I"$PRIMME_ROOT/include"     -I"$PYTHON_ROOT/include/python3.8"     "$TEST_DIR/benchmark_n2_sto6g_fci_14400_ipt.cu"     "$PRIMME_ROOT/lib/libprimme.a"     -o "$TEST_DIR/benchmark_n2_sto6g_fci_14400_ipt_cuda"     -lcublas -lcusparse -lcusolver -lcudart     -L"$PYTHON_ROOT/lib" -lpython3.8     -L"$TEST_DIR" -lopenblas     -lpthread -ldl -lutil -lrt     -Xlinker -rpath -Xlinker "$GCC_LIB_DIR"     -Xlinker -rpath -Xlinker "$PYTHON_ROOT/lib"     -Xlinker -rpath -Xlinker "$OPENBLAS_DIR"     -lgfortran "$GCC_LIB_DIR/libstdc++.so" -lm
set +x
echo "===== run ====="
"$TEST_DIR/benchmark_n2_sto6g_fci_14400_ipt_cuda"
status=$?
echo "===== output files ====="
