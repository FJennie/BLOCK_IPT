...
#define DEFAULT_NS "16384,26000,32768"
...
static void load_config(Config *cfg)
{
    ...
    if (cfg->results_dir == NULL) {
        ...
        snprintf(default_results, sizeof(default_results),
                 "%s/results/mixed_precision_tf32", cfg->root);
    ...
    if (cfg->logs_dir == NULL) {
        ...
        snprintf(default_logs, sizeof(default_logs),
                 "%s/logs/mixed_precision_tf32", cfg->root);
    ...
    cfg->run_fp16 = parse_bool_env("IPT_RUN_FP16", 0);
    cfg->run_tf32 = parse_bool_env("IPT_RUN_TF32", 1);
    if (!cfg->run_fp16 && !cfg->run_tf32) {
        fprintf(stderr, "at least one of IPT_RUN_FP16 or IPT_RUN_TF32 must be true\n");
        exit(2);
    }

    {
        ...
        if (summary != NULL && summary[0] != '\0') {
            ...
            snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                     "%s/tf32_initial_cusolver_sweep_summary.txt",
                     cfg->results_dir);
        }
    }
}
...
static MixedResult run_mixed_tf32(cublasHandle_t blas_handle,
                                  const double *d_matrix, int n,
                                  const Config *cfg, int compute_residuals)
{
    MixedResult result;
    ...
    result.fp16_time_sec = device_result.tf32_time_sec;
    result.fp64_time_sec = device_result.fp64_time_sec;
    result.total_time_sec = device_result.total_time_sec;
    result.fp16_iterations = device_result.tf32_iterations;
    result.fp64_iterations = device_result.fp64_iterations;
    result.fp16_fixed_point_residual =
...
static MixedResult run_mixed_tf32_average(cublasHandle_t blas_handle,
                                          const double *d_matrix, int n,
                                          const Config *cfg)
{
    MixedResult average;
    ...
    printf("Mixed TF32->FP64 warmup for N=%d\n", n);
    {
        MixedResult warm = run_mixed_tf32(blas_handle, d_matrix, n, cfg, 0);
        if (strcmp(warm.status, "ok") != 0) {
            mixed_result_fail(&average, "failed", warm.error);
            average.fp16_time_sec = warm.fp16_time_sec;
            average.fp64_time_sec = warm.fp64_time_sec;
    ...
    for (int rep = 1; rep <= cfg->repeats; ++rep) {
        MixedResult one =
            run_mixed_tf32(blas_handle, d_matrix, n, cfg,
                           cfg->compute_residuals && rep == cfg->repeats);

        printf("  mixed run %d/%d: tf32 = %.8g s, fp64 = %.8g s, "
               "total = %.8g s, tf32_fp = %.8g, fp64_fp = %.8g, "
               "tf32_iters = %d, fp64_iters = %d, residual = %.8g, "
               "status = %s",
               rep, cfg->repeats, one.fp16_time_sec, one.fp64_time_sec,
               one.total_time_sec, one.fp16_fixed_point_residual,
               one.fp64_fixed_point_residual, one.fp16_iterations,
                            ...
                            size_t size)
{
    ...
    snprintf(path, size, "%s/tf32_initial_cusolver_sweep_epsilon_%s.csv",
...
static int csv_has_requested_case(const Config *cfg, const char *csv_path,
                                  int n)
{
    if (cfg->run_fp16 && !csv_has_row_for_precision(csv_path, n, "fp16")) {
        return 0;
    }
    if (cfg->run_tf32 && !csv_has_row_for_precision(csv_path, n, "tf32")) {
        return 0;
    }
...
static void write_summary_header(const Config *cfg, const char *gpu_name)
{
    ...
    fprintf(fp, "IPT_C mixed precision sweep: FP16/TF32 initial + FP64 "
    ...
    fprintf(fp, "tf32_tol = %.17g\n", cfg->tf32_tol);
    fprintf(fp, "tf32_maxiter = %d\n", cfg->tf32_maxiter);
    ...
    fprintf(fp, "run_fp16 = %s\n", cfg->run_fp16 ? "true" : "false");
    fprintf(fp, "run_tf32 = %s\n", cfg->run_tf32 ? "true" : "false");
                   ...
                   const Config *cfg)
{
    ...
    if (cfg->run_tf32) {
        (void)run_mixed_tf32(blas_handle, d_matrix, warm_n, cfg, 0);
    }
...
int main(void)
{
    ...
    printf("===== IPT_C mixed precision FP16/TF32 sweep vs cuSolver =====\n");
    printf("ROOT = %s\n", cfg.root);
    printf("RESULT_DIR = %s\n", cfg.results_dir);
    ...
    for (int eps_idx = 0; eps_idx < cfg.eps_count; ++eps_idx) {
        ...
        for (int n_idx = 0; n_idx < cfg.n_count; ++n_idx) {
            ...
            if (cfg.skip_existing && csv_has_requested_case(&cfg, csv_path, n)) {
                ...
                printf("Skipping epsilon=%.12g N=%d because requested rows "
                       "already exist in %s\n",
                       epsilon, n, csv_path);
                continue;
            }
            ...
            if (make_host_matrix(&h_matrix, n, epsilon,
                ...
                snprintf(status_tf32, sizeof(status_tf32), "failed");
                snprintf(error_fp16, sizeof(error_fp16),
                         "host matrix allocation failed");
                snprintf(error_tf32, sizeof(error_tf32),
                         "host matrix allocation failed");
                failures++;
                goto row_done;
            }
            ...
            if (copy_matrix_to_device(&d_matrix, h_matrix, n, copy_error,
                ...
                snprintf(error_tf32, sizeof(error_tf32), "%s", copy_error);
                failures++;
                goto row_done;
            }
            ...
            if (cfg.run_tf32) {
                printf("Running mixed TF32->FP64 for epsilon=%.12g N=%d\n",
                       epsilon, n);
                mixed_tf32 = run_mixed_tf32_average(blas_handle, d_matrix, n, &cfg);

                fp64_converged_tf32 =
                    isfinite(mixed_tf32.fp64_fixed_point_residual) &&
                    mixed_tf32.fp64_fixed_point_residual <= cfg.fp64_tol;
                if (strcmp(syevd.status, "ok") != 0 ||
                    strcmp(original_ipt.status, "ok") != 0 ||
                    strcmp(mixed_tf32.status, "ok") != 0) {
                    snprintf(status_tf32, sizeof(status_tf32), "partial_failed");
                    join_errors(error_tf32, sizeof(error_tf32), syevd.error,
                                original_ipt.error, mixed_tf32.error);
                    if (strcmp(original_ipt.status, "ok") != 0 ||
                        strcmp(mixed_tf32.status, "ok") != 0) {
                        failures++;
                    }
                } else if (!fp64_converged_tf32) {
                    snprintf(status_tf32, sizeof(status_tf32), "not_converged");
                    snprintf(error_tf32, sizeof(error_tf32),
                             "fp64_fixed_point_residual %.17g > tol %.17g",
                             mixed_tf32.fp64_fixed_point_residual, cfg.fp64_tol);
                }

                printf("Averages TF32: syevd = %.8g s, original_ipt = %.8g s, "
                       "mixed_total = %.8g s, "
                       "mixed_residual = %.8g, initial_fp = %.8g, fp64_fp = %.8g, "
                       "tf32_iters = %d, fp64_iters = %d, status = %s\n",
                       syevd.time_sec, original_ipt.time_sec,
                       mixed_tf32.total_time_sec,
                       mixed_tf32.residual,
                       mixed_tf32.fp16_fixed_point_residual,
                       mixed_tf32.fp64_fixed_point_residual,
                       mixed_tf32.fp16_iterations,
                       mixed_tf32.fp64_iterations, status_tf32);
            }
        ...
        row_done:
            ...
            if (cfg.run_tf32) {
                append_csv_row(csv_path, &cfg, n, epsilon, &syevd, &original_ipt,
                               &mixed_tf32, "tf32", cfg.tf32_tol,
                               cfg.tf32_maxiter, gpu_name, status_tf32,
                               error_tf32);
                snprintf(summary_line, sizeof(summary_line),
                         "epsilon=%.12g N=%d initial_precision=tf32 "
                         "finished_at=%s status=%s "
                         "time_syevd=%.17g residual_syevd=%.17g "
                         "time_original_ipt=%.17g residual_original_ipt=%.17g "
                         "original_ipt_iterations=%d original_ipt_fp=%.17g "
                         ...
                         "fp64_iterations=%d initial_fp=%.17g fp64_fp=%.17g "
                         "repeats=%d per_case_warmup=true csv=%s",
                         epsilon, n, time_buf, status_tf32, syevd.time_sec,
                         syevd.residual, original_ipt.time_sec,
                         original_ipt.residual, original_ipt.iterations,
                         original_ipt.fixed_point_residual,
                         mixed_tf32.fp16_time_sec,
                         mixed_tf32.fp64_time_sec, mixed_tf32.total_time_sec,
                         mixed_tf32.residual, mixed_tf32.fp16_iterations,
                         mixed_tf32.fp64_iterations,
                         mixed_tf32.fp16_fixed_point_residual,
                         mixed_tf32.fp64_fixed_point_residual, cfg.repeats,
                         csv_path);
                append_summary(&cfg, summary_line);
            }
            cudaFree(d_matrix);
