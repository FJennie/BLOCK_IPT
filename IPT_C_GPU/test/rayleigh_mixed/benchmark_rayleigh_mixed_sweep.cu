...
static void load_config(Config *cfg)
{
    ...
    if (summary != NULL && summary[0] != '\0') {
        ...
        snprintf(cfg->summary_txt, sizeof(cfg->summary_txt),
                 "%s/ipt_mixed_tf32_rayleigh_sweep_summary.txt",
                 cfg->results_dir);
    }
}
...
static RayleighRunResult run_mixed_tf32(cublasHandle_t handle,
...
static RayleighRunResult run_mixed_tf32_average(cublasHandle_t handle,
                                                const double *d_matrix, int n,
                                                const Config *cfg)
{
    ...
    printf("Mixed TF32 IPT warmup for N=%d\n", n);
    {
        RayleighRunResult warm = run_mixed_tf32(handle, d_matrix, n, cfg, 0);
    ...
    for (int rep = 1; rep <= cfg->repeats; ++rep) {
        ...
        RayleighRunResult one =
            run_mixed_tf32(handle, d_matrix, n, cfg, compute_residual);

        printf("  mixed tf32 run %d/%d: tf32=%.8g s, fp64=%.8g s, "
               "total=%.8g s, tf32_fp=%.8g, fp64_fp=%.8g, abs_res=%.8g, "
               "iters=%d/%d, status=%s",
               rep, cfg->repeats, one.tf32_time_sec, one.fp64_time_sec,
               one.total_time_sec, one.tf32_fixed_point_residual,
                                              ...
                                              const Config *cfg)
{
    ...
    rayleigh_run_result_reset(&average, "ok");
    printf("Rayleigh mixed QR warmup for N=%d\n", n);
    {
...
{
    (void)epsilon;
    snprintf(path, size, "%s/ipt_mixed_tf32_rayleigh_sweep.csv",
             cfg->results_dir);
}
...
static int csv_has_case(const char *csv_path, int n, double epsilon)
{
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (n_text != NULL && epsilon_text != NULL && atoi(n_text) == n &&
            fabs(atof(epsilon_text) - epsilon) <= 1.0e-12) {
            rows_for_case++;
        }
    }
    fclose(fp);
    return rows_for_case >= 2;
}

static void append_csv_row(const char *csv_path, const Config *cfg, int n,
                           double epsilon, const char *method,
                           const RayleighRunResult *result,
                           const char *gpu_name)
{
    ...
    snprintf(method_clean, sizeof(method_clean), "%s", method);
    ...
    csv_clean(method_clean);
    csv_clean(gpu_clean);
    csv_clean(status_clean);
    csv_clean(error_clean);

    fprintf(fp,
            "%d,%.12g,%s,%.17g,%.17g,%d,%d,%d,"
            "%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%d,%d,"
            "%s,%s,%s,%s,%s\n",
            n, epsilon, method_clean, cfg->tf32_tol, cfg->fp64_tol,
            cfg->tf32_maxiter, cfg->fp64_maxiter, cfg->repeats,
...
static void warmup(cublasHandle_t handle, const Config *cfg)
{
    ...
    (void)run_mixed_tf32(handle, d_matrix, cfg->warmup_n, &warm_cfg, 0);
...
int main(void)
{
    ...
    result_csv_path(&cfg, 0.0, csv_path, sizeof(csv_path));
    ...
    snprintf(summary_line, sizeof(summary_line),
             "started_at=%s methods=mixed_tf32_ipt,rayleigh_mixed_qr "
             "repeats=%d tf32_tol=%.17g fp64_tol=%.17g gpu=%s",
             time_buf, cfg.repeats, cfg.tf32_tol, cfg.fp64_tol, gpu_name);
    append_summary(&cfg, summary_line);

    printf("===== IPT mixed TF32 and Rayleigh mixed sweep =====\n");
    ...
    for (int eps_idx = 0; eps_idx < cfg.eps_count; ++eps_idx) {
        ...
        for (int n_idx = 0; n_idx < cfg.n_count; ++n_idx) {
            ...
            double *h_matrix = NULL;
            double *d_matrix = NULL;
            RayleighRunResult mixed_result;
            RayleighRunResult rayleigh_result;
            char error[ERROR_LEN] = "";
            double matrix_start = 0.0;

            rayleigh_run_result_reset(&mixed_result, "not_run");
            rayleigh_run_result_reset(&rayleigh_result, "not_run");
            if (cfg.skip_existing && csv_has_case(csv_path, n, epsilon)) {
                printf("Skipping epsilon=%.12g N=%d because rows exist in %s\n",
                       epsilon, n, csv_path);
                continue;
            }
            ...
            if (make_host_matrix(&h_matrix, n, epsilon,
                ...
                snprintf(mixed_result.status, sizeof(mixed_result.status),
                         "failed");
                snprintf(mixed_result.error, sizeof(mixed_result.error),
                         "host matrix allocation failed");
                snprintf(rayleigh_result.status, sizeof(rayleigh_result.status),
                         "failed");
                snprintf(rayleigh_result.error, sizeof(rayleigh_result.error),
                         "host matrix allocation failed");
                failures += 2;
                goto row_done;
            }
            ...
            if (copy_matrix_to_device(&d_matrix, h_matrix, n, error,
                ...
                snprintf(mixed_result.status, sizeof(mixed_result.status),
                         "failed");
                snprintf(mixed_result.error, sizeof(mixed_result.error), "%s",
                         error);
                snprintf(rayleigh_result.status, sizeof(rayleigh_result.status),
                         "failed");
                snprintf(rayleigh_result.error, sizeof(rayleigh_result.error),
            ...
            mixed_result = run_mixed_tf32_average(handle, d_matrix, n, &cfg);
            if (strcmp(mixed_result.status, "ok") != 0) {
                failures++;
            }

            rayleigh_result = run_rayleigh_average(handle, d_matrix, n, &cfg);
            if (strcmp(rayleigh_result.status, "ok") != 0) {
                failures++;
            }

        row_done:
            append_csv_row(csv_path, &cfg, n, epsilon, "mixed_tf32_ipt",
                           &mixed_result, gpu_name);
            append_csv_row(csv_path, &cfg, n, epsilon, "rayleigh_mixed_qr",
                           &rayleigh_result, gpu_name);
            wall_time_string(time_buf, sizeof(time_buf));
            snprintf(summary_line, sizeof(summary_line),
                     "epsilon=%.12g N=%d method=mixed_tf32_ipt "
                     "finished_at=%s status=%s "
                     "time_total=%.17g tf32_time=%.17g fp64_time=%.17g "
                     "abs_residual=%.17g tf32_fp=%.17g fp64_fp=%.17g "
                     "iters=%d/%d csv=%s",
                     epsilon, n, time_buf, mixed_result.status,
                     mixed_result.total_time_sec, mixed_result.tf32_time_sec,
                     mixed_result.fp64_time_sec, mixed_result.absolute_residual,
                     mixed_result.tf32_fixed_point_residual,
                     mixed_result.fp64_fixed_point_residual,
                     mixed_result.tf32_iterations,
                     mixed_result.fp64_iterations, csv_path);
            append_summary(&cfg, summary_line);
            snprintf(summary_line, sizeof(summary_line),
                     "epsilon=%.12g N=%d method=rayleigh_mixed_qr "
                     "finished_at=%s status=%s "
                     "time_total=%.17g tf32_time=%.17g orth_time=%.17g "
                     "transform_time=%.17g fp64_time=%.17g back_time=%.17g "
                     "abs_residual=%.17g tf32_fp=%.17g fp64_fp=%.17g "
                     "iters=%d/%d csv=%s",
                     epsilon, n, time_buf, rayleigh_result.status,
                     rayleigh_result.total_time_sec,
                     rayleigh_result.tf32_time_sec,
                     rayleigh_result.orthogonalize_time_sec,
                     rayleigh_result.rayleigh_transform_time_sec,
                     rayleigh_result.fp64_time_sec,
                     rayleigh_result.backtransform_time_sec,
                     rayleigh_result.absolute_residual,
                     rayleigh_result.tf32_fixed_point_residual,
                     rayleigh_result.fp64_fixed_point_residual,
                     rayleigh_result.tf32_iterations,
                     rayleigh_result.fp64_iterations, csv_path);
            append_summary(&cfg, summary_line);
            cudaFree(d_matrix);
            free(h_matrix);
            cudaDeviceSynchronize();
